...
const useBoardData = () => {
    ...
    const addPost = (newPost) => {
        const config = {
            method: 'post',
            url: 'http://52.62.34.185:8080/api/notice_boards',
            data: newPost,
            headers: {
    ...
    const updatePost = async (id, updatePost) => {
        ...
        try {
            const response = await axios.put(`http://52.62.34.185:8080/api/notice_boards/${id}`, updatePost, {
                headers: {
                    'Content-Type': 'application/json'
                }
            });
    ...
    const deletePost = (id, password) => {
        const url = `http://52.62.34.185:8080/api/notice_boards/${id}?password=${encodeURIComponent(password)}`;
    ...
    const verifyPassword = async (id, password) => {
        try {
            const response = await axios.get(`http://52.62.34.185:8080/api/notice_boards/${id}/verify?password=${encodeURIComponent(password)}`);
